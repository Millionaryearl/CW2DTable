...
#import "CW2DTableVC3.h"
#import "CW2DTableCell.h"

@interface CW2DTableVC3 ()
...
-(void)refreshSrchRlt{
     ...
     *
     *  section1    col1             col2                              col3                             ...
     *  section2      -                -            subCol1           subCol2          subCol3          ...
     *  section3   row-h1           row-c1          row-c2            row-c3           row-c4           ...
     *  section4      -             row-c1          row-c2            row-c3           row-c4           ...
     *      .         -             row-c1          row-c2            row-c3           row-c4           ...
     *      .      row-h2           row-c1          row-c2            row-c3           row-c4           ...
     *      .        ...
     */
    NSArray *columnNames = @[@"属性",@"大类",@"波段1",@"波段2",@"波段3",@"波段4"];
    NSMutableArray *columnSubname = [[NSMutableArray alloc]init];
    for (int i=0; i<[columnNames count]-2; i++) {
        [columnSubname addObject:@"计划定量"];
        [columnSubname addObject:@"实际定量"];
        [columnSubname addObject:@"主推SKC"];
    }
    ...
    [_dataSrcArrNumVersion addObject:columnSubname];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section ==0) {
        //for column header
        [cell setBackgroundColor:[UIColor colorWithRed:(float)181/255 green:(float)251/255 blue:(float)251/255 alpha:1]];
    }
    ...
    if (indexPath.row ==1 &&[self.collectionView numberOfItemsInSection:indexPath.section]==[self.collectionView numberOfItemsInSection:2]){
        //for subcrow header
        [cell setBackgroundColor:[UIColor colorWithRed:(float)181/255 green:(float)251/255 blue:(float)251/255 alpha:1]];
    }
    return cell;
